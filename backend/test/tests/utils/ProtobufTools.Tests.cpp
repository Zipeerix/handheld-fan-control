...
#include "test/proto/ProtoForTests.pb.h"
...
TEST(ProtobufTools, createProtoPayload_FormatsStringCorrectly) {
    ...
    const auto payload = hfc::utils::createProtoPayload(type_name, serialized_data);
...
TEST(ProtobufTools, makePayloadFromProto_SerializesHelloWorldMessage) {
    hfc::test_proto::HelloWorld msg;
    msg.set_message("Hello, Protobuf!");
    msg.set_integer(123);

    const auto payload = hfc::utils::makePayloadFromProto(msg);
    const std::string expected_prefix = "hfc.test_proto.HelloWorld:";
    ...
    hfc::test_proto::HelloWorld deserialized_msg;
...
TEST(ProtobufTools, makePayloadFromProto_SerializesEmptyHelloWorldMessage) {
    const hfc::test_proto::HelloWorld msg;
...
TEST(ProtobufTools, makeProtoFromPayload_DeserializesHelloWorldMessage) {
    hfc::test_proto::HelloWorld original_msg;
    original_msg.set_message("Test Message");
    original_msg.set_integer(42);
    std::string serialized_data;
    original_msg.SerializeToString(&serialized_data);
    const std::string payload =
        hfc::utils::createProtoPayload(hfc::test_proto::HelloWorld::descriptor()->full_name(), serialized_data);

    const auto result = hfc::utils::makeProtoFromPayload<hfc::test_proto::HelloWorld>(payload);
...
TEST(ProtobufTools, makeProtoFromPayload_DeserializesEmptyHelloWorldMessage) {
    const hfc::test_proto::HelloWorld original_msg;  // Empty message
    std::string serialized_data;
    original_msg.SerializeToString(&serialized_data);
    const std::string payload =
        hfc::utils::createProtoPayload(hfc::test_proto::HelloWorld::descriptor()->full_name(), serialized_data);
...
TEST(ProtobufTools, makeProtoFromPayload_ReturnsInvalidFormatError_WhenNoDelimiter) {
    const std::string payload = "hfc.test_proto.HelloWorld_NoDelimiter";  // Missing colon

    const auto result = hfc::utils::makeProtoFromPayload<hfc::test_proto::HelloWorld>(payload);

    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::InvalidFormat);
    ASSERT_EQ(result.error().message, "Received message: hfc.test_proto.HelloWorld_NoDelimiter");
}

TEST(ProtobufTools, makeProtoFromPayload_ReturnsInvalidFormatError_WhenLongPayloadTruncatedInErrorMsg) {
    std::string long_payload = "hfc.test_proto.HelloWorld";
    long_payload += std::string(200, 'A');  // Make it longer than 128 chars

    const auto result = hfc::utils::makeProtoFromPayload<hfc::test_proto::HelloWorld>(long_payload);

    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::InvalidFormat);
    ASSERT_TRUE(result.error().message.rfind("Received message: hfc.test_proto.HelloWorldA", 0) == 0);
...
TEST(ProtobufTools, makeProtoFromPayload_ReturnsTypeMismatchError_WhenExpectedTypeDiffers) {
    ...
    const std::string payload =
        hfc::utils::createProtoPayload(google::protobuf::Empty::descriptor()->full_name(), serialized_data);

    const auto result = hfc::utils::makeProtoFromPayload<hfc::test_proto::HelloWorld>(payload);

    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::TypeMismatch);
    ASSERT_EQ(result.error().message,
              std::format("Received {} but expected {}",
                          google::protobuf::Empty::descriptor()->full_name(),
                          hfc::test_proto::HelloWorld::descriptor()->full_name()));
}

TEST(ProtobufTools, makeProtoFromPayload_ReturnsDeserializationFailedError_WhenCorruptBinaryData) {
    const std::string payload = hfc::utils::createProtoPayload(hfc::test_proto::HelloWorld::descriptor()->full_name(),
                                                               "corrupt_binary_data_that_is_not_protobuf");

    const auto result = hfc::utils::makeProtoFromPayload<hfc::test_proto::HelloWorld>(payload);

    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::DeserializationFailed);
}

TEST(ProtobufTools, makeProtoFromPayload_ReturnsDeserializationFailedError_WhenEmptyBinaryDataForNonEmptyMessage) {
    const std::string payload =
        hfc::utils::createProtoPayload(hfc::test_proto::HelloWorld::descriptor()->full_name(), "");
...
TEST(ProtobufTools, makeBaseProtoFromPayload_DeserializesHelloWorldMessageDynamically) {
    ...
    const auto result = hfc::utils::makeBaseProtoFromPayload(payload);

    ASSERT_TRUE(result.has_value());
    ASSERT_NE(result->get(), nullptr);

    const auto* deserialized_msg = dynamic_cast<const hfc::test_proto::HelloWorld*>(result->get());
...
TEST(ProtobufTools, makeBaseProtoFromPayload_DeserializesEmptyHelloWorldMessageDynamically) {
    const hfc::test_proto::HelloWorld original_msg;
    std::string serialized_data;
    original_msg.SerializeToString(&serialized_data);
    const std::string payload =
        hfc::utils::createProtoPayload(hfc::test_proto::HelloWorld::descriptor()->full_name(), serialized_data);
...
TEST(ProtobufTools, makeBaseProtoFromPayload_ReturnsInvalidFormatError_WhenNoDelimiter) {
    const std::string payload = "hfc.test_proto.HelloWorld_NoDelimiter_Base";  // Missing colon

    const auto result = hfc::utils::makeBaseProtoFromPayload(payload);

    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::InvalidFormat);
    ASSERT_EQ(result.error().message, "Received message: hfc.test_proto.HelloWorld_NoDelimiter_Base");
}

TEST(ProtobufTools, makeBaseProtoFromPayload_ReturnsDeserializationFailedError_WhenUnknownMessageType) {
    const std::string payload = hfc::utils::createProtoPayload("non.existent.MessageType", "some_data");
...
TEST(ProtobufTools, makeBaseProtoFromPayload_ReturnsDeserializationFailedError_WhenCorruptBinaryData) {
    ...
    ASSERT_FALSE(result.has_value());
    ASSERT_EQ(result.error().type, hfc::utils::ProtoPayloadParseError::DeserializationFailed);
    ASSERT_TRUE(result.error().message.rfind("Unable to deserialize as hfc.test_proto.HelloWorld. Message:", 0) == 0);
}
