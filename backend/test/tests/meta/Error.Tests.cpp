...
TEST(Error, IsEnum_RecognizesEnumClass) {
    ASSERT_TRUE(hfc::IsEnum<TestErrorType>);
}
...
TEST(Error, IsEnum_RecognizesPlainEnum) {
    ...
    ASSERT_TRUE(hfc::IsEnum<PlainEnum>);
}

TEST(Error, IsEnum_RejectsNonEnumTypes) {
    ASSERT_FALSE(hfc::IsEnum<int>);
    ASSERT_FALSE(hfc::IsEnum<std::string>);
    ASSERT_FALSE(hfc::IsEnum<double>);

    struct MyStruct {};

    ASSERT_FALSE(hfc::IsEnum<MyStruct>);
}

TEST(Error, DefaultConstructor_InitializesCorrectly) {
    const auto error = hfc::Error<TestErrorType>();
...
TEST(Error, Constructor_WithOnlyErrorType_InitializesCorrectly) {
    const auto error = hfc::Error<TestErrorType>(TestErrorType::FileNotFound);
...
TEST(Error, Constructor_WithErrorTypeAndMessage_InitializesCorrectly) {
    ...
    const auto error = hfc::Error<TestErrorType>(TestErrorType::FileNotFound, errorMessage);
    ASSERT_EQ(error.type, TestErrorType::FileNotFound);
    ASSERT_EQ(error.message, errorMessage);
}

TEST(Error, Constructor_WithErrorTypeAndEmptyMessage_InitializesCorrectly) {
    const auto error = hfc::Error<TestErrorType>(TestErrorType::PermissionDenied, "");
...
TEST(Error, FormatsError_WithNonEmptyMessage) {
    const auto error = hfc::Error<TestErrorType>(TestErrorType::NetworkError, "Host unreachable.");
    const std::string formatted_string = std::format("{}", error);
...
TEST(Error, FormatsError_WithLongMessage) {
    ...
    const auto error = hfc::Error<TestErrorType>(TestErrorType::Unknown, long_message);
    const std::string formatted_string = std::format("{}", error);
...
TEST(Error, FormatsError_WithSpecialCharactersInMessage) {
    const auto error =
        hfc::Error<TestErrorType>(TestErrorType::PermissionDenied, "Access denied to 'C:\\path\\file.ini'. Code: 0x5.");
    const std::string formatted_string = std::format("{}", error);
...
TEST(Error, FormatsError_WhenErrorTypeIsZeroValue) {
    const auto error = hfc::Error<TestErrorType>(static_cast<TestErrorType>(0), "Default error.");
    const std::string formatted_string = std::format("{}", error);
